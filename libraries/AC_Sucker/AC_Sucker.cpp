...
//  # define AP_AVOID_BEHAVE_DEFAULT AC_Sucker::BehaviourType::BEHAVIOR_STOP
// #else
//  # define AP_AVOID_BEHAVE_DEFAULT AC_Sucker::BehaviourType::BEHAVIOR_SLIDE
// #endif

const AP_Param::GroupInfo AC_Sucker::var_info[] = {
    ...
    // @User: Standard
    AP_GROUPINFO("ENABLE", 1,  AC_Sucker, _enabled, AC_SUCKER_DEFAULT),

    // @Param: IGNORE_ANGLE_MIN
    // @DisplayName: Ignore minimum angle
    // @Description: Min angle to ignore
    // @Units: cdeg
    // @Range: 0 4500
    // @User: Standard
    AP_GROUPINFO("IGNORE_ANGLE_MIN", 2,  AC_Sucker, _ignore_angle_min, 1000),
    ...
    // // @Range: 1 30
    // // @User: Standard
    // AP_GROUPINFO("DIST_MAX", 3,  AC_Sucker, _dist_max, AC_SUCKER_NONGPS_DIST_MAX_DEFAULT),
    ...
    // // @Range: 1 10
    // // @User: Standard
    // AP_GROUPINFO("MARGIN", 4, AC_Sucker, _margin, 2.0f),
    ...
    // // @User: Standard
    // AP_GROUPINFO("BEHAVE", 5, AC_Sucker, _behavior, AP_AVOID_BEHAVE_DEFAULT),
...
// void AC_Sucker::adjust_velocity(float kP, float accel_cmss, Vector2f &desired_vel_cms, float dt)
// {
//     // exit immediately if disabled
//     if (_enabled == AC_SUCKER_DISABLED) {
//         return;
//     }

//     // limit acceleration
//     const float accel_cmss_limited = MIN(accel_cmss, AC_SUCKER_ACCEL_CMSS_MAX);

//     if ((_enabled & AC_SUCKER_STOP_AT_FENCE) > 0) {
...
//     if ((_enabled & AC_SUCKER_STOP_AT_BEACON_FENCE) > 0) {
//         adjust_velocity_beacon_fence(kP, accel_cmss_limited, desired_vel_cms, dt);
//     }

//     if ((_enabled & AC_SUCKER_USE_PROXIMITY_SENSOR) > 0 && _proximity_enabled) {
...
// void AC_Sucker::adjust_velocity(float kP, float accel_cmss, Vector3f &desired_vel_cms, float dt)
// {
...
// void AC_Sucker::adjust_speed(float kP, float accel, float heading, float &speed, float dt)
// {
...
// void AC_Sucker::adjust_velocity_z(float kP, float accel_cmss, float& climb_rate_cms, float dt)
// {
//     // exit immediately if disabled
//     if (_enabled == AC_SUCKER_DISABLED) {
//         return;
...
//     if ((_enabled & AC_SUCKER_STOP_AT_FENCE) > 0 && fence && (fence->get_enabled_fences() & AC_FENCE_TYPE_ALT_MAX) > 0) {
...
// void AC_Sucker::adjust_roll_pitch(float &roll, float &pitch, float veh_angle_max)
// {
...
//     const float angle_limit = constrain_float(_angle_max, 0.0f, veh_angle_max * AC_SUCKER_ANGLE_MAX_PERCENT);
...
//  */
// void AC_Sucker::limit_velocity(float kP, float accel_cmss, Vector2f &desired_vel_cms, const Vector2f& limit_direction, float limit_distance_cm, float dt)
// {
...
//  */
// float AC_Sucker::get_max_speed(float kP, float accel_cmss, float distance_cm, float dt) const
// {
//     if (is_zero(kP)) {
...
//  */
// void AC_Sucker::adjust_velocity_circle_fence(float kP, float accel_cmss, Vector2f &desired_vel_cms, float dt)
// {
//     AC_Fence *fence = AP::fence();
//     if (fence == nullptr) {
//         return;
...
//     if ((AC_Sucker::BehaviourType)_behavior.get() == BEHAVIOR_SLIDE) {
...
//  */
// void AC_Sucker::adjust_velocity_inclusion_and_exclusion_polygons(float kP, float accel_cmss, Vector2f &desired_vel_cms, float dt)
// {
...
//  */
// void AC_Sucker::adjust_velocity_inclusion_circles(float kP, float accel_cmss, Vector2f &desired_vel_cms, float dt)
// {
//     const AC_Fence *fence = AP::fence();
//     if (fence == nullptr) {
//         return;
...
//     switch ((AC_Sucker::BehaviourType)_behavior.get()) {
//         case BEHAVIOR_SLIDE:
...
//  */
// void AC_Sucker::adjust_velocity_proximity(float kP, float accel_cmss, Vector2f &desired_vel_cms, float dt)
// {
...
//  */
// float AC_Sucker::get_stopping_distance(float kP, float accel_cmss, float speed_cms) const
// {
...
// float AC_Sucker::distance_to_lean_pct(float dist_m)
// {
...
// void AC_Sucker::get_proximity_roll_pitch_pct(float &roll_positive, float &roll_negative, float &pitch_positive, float &pitch_negative)
// {
//     AP_Proximity *proximity = AP::proximity();
...
// singleton instance
AC_Sucker *AC_Sucker::_singleton;

namespace AP {

AC_Sucker *ac_sucker()
{
    return AC_Sucker::get_singleton();
}
